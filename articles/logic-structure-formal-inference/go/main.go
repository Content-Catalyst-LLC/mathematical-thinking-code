...
func main() {
	path := filepath.Join("..", "data", "raw", "derivation_dependencies.csv")
	...
	fmt.Println("Derivation dependency adjacency list")
	for node, targets := range graph {
		fmt.Printf("%s -> %v\n", node, targets)
	}

	fmt.Println("Interpretation: derivation graphs make inference dependencies visible.")
}

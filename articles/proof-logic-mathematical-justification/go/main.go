...
func main() {
	...
	fmt.Println("Proof dependency adjacency list")
	for node, targets := range graph {
		fmt.Printf("%s -> %v\n", node, targets)
	}

	fmt.Println("Interpretation: dependency graphs help inspect proof architecture; they do not replace proof checking.")
}

...
traditions <- read.csv(file.path(data_dir, "proof_traditions.csv"), stringsAsFactors = FALSE)
milestones <- read.csv(file.path(data_dir, "proof_milestones.csv"), stringsAsFactors = FALSE)
...
tradition_summary$interpretation <- "synthetic milestone count supports teaching, not historical prevalence"

warning_summary <- data.frame(
  topic = warnings$topic,
  warning = warnings$warning,
  mitigation = warnings$mitigation,
  interpretation = "responsible proof history requires attention to canon, translation, medium, and power"
)
...
cat("Proof-history R summaries written.\n")
